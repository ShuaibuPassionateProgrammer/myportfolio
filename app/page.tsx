	...
	return (
		<main className="max-w-5xl mx-auto px-4 pt-6 pb-10 space-y-10 sm:space-y-14">
			<header className="sticky top-4 z-50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-lg flex items-center justify-between px-4 sm:px-8 py-2 sm:py-3 rounded-2xl shadow-xl mb-4 border border-slate-200 dark:border-slate-800 transition-all">
				...
				{/* Mobile Nav Toggle */}
				<button
					className="md:hidden p-2 rounded-lg hover:bg-indigo-50 dark:hover:bg-slate-800 transition"
				...
				{/* Mobile Nav Drawer */}
				{mobileOpen && (
					<nav className="absolute top-full left-0 w-full bg-white/95 dark:bg-slate-900/95 border-t border-slate-200 dark:border-slate-800 flex flex-col items-center gap-2 py-4 shadow-2xl md:hidden animate-fade-in-down rounded-b-2xl">
						{navLinks.map((link) => (
							<a
								key={link.href}
								href={link.href}
